...
import AssociatedAuthors from "shared/components/common/associatedAuthors";
    ...
    <>
      ...
      <div className={classNames(styles.topLevelContainer)} ref={bodyRef}>
        ...
        >
          ...
          >
            ...
            >
              <img
                alt="profile-pic"
                src={
                  publisherDetail?.publishing_logo
                    ? publisherDetail?.publishing_logo
                    : defaultAvatar.src
                }
                width={800}
              ...
              >
                ...
                >
                  <label className={classNames(styles.publisherName)}>
                    {publisherDetail?.publishing_house}
                  </label>
            ...
            >
              {/* <CustomButton
                ...
                onClick={() => {
                  window.open(
                    publisherDetail?.website.startsWith("http")
                      ? publisherDetail?.website
                      : "https://" + publisherDetail?.website,
                    "_blank"
                  );
                }}
              /> */}
          ...
          {associatedData?.data?.data?.length > 0 ? (
            <div
              className={classNames(
            ...
            >
              <label className={classNames(styles.title)}>
                Associated <label>Authors</label>
              </label>
              <AssociatedAuthors authors={associatedData} />
            </div>
          ) : null}
          ...
          >
            <label className={classNames(styles.title)}>
              Books Published By{" "}
              <span>{publisherDetail?.publishing_house}</span>
            </label>
            <div
              className={classNames(
...
export const getServerSideProps = withError(async ({ req, res, params }) => {
  const [profileRes, associatedRes] = await Promise.all([
    fetch(BaseURL + Endpoint.kid.publisher.profile + params?.id, {
      cache: "no-cache",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
      },
    }),
    fetch(BaseURL + Endpoint.kid.publisher.associatedPartner + params?.id, {
      cache: "no-cache",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
      },
  ...
  const [profileData, associatedData] = await Promise.all([
    profileRes.json(),
    associatedRes.json(),
  ]);
