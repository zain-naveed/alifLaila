...
import { sharedBookList } from "shared/services/kid/bookService";
...
const SharedBooks = () => {
  ...
  const handleGetMyBooksList = (sort: any) => {
    setLoading(true);
    sharedBookList(search, currentPage, sort)
  ...
  return (
    <>
      ...
      >
        ...
        >
          <label className={classNames(styles.title)}>Shared books</label>
          <label className={classNames(styles.subTitle)}>
            See all your shared Books
          </label>
        </div>
        <div
          className={classNames(
